...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const scanAndObserve = () => {
    document.querySelectorAll(".fade-in:not(.is-loaded)").forEach(
...
// src/js/utils/generateImgTag.js
function generateImgTag(imageDataRaw, options = {}) {
  ...
  const sources = Object.entries(imageData).map(([format2, variants]) => {
    const srcset = variants.map((v) => {
      const url = v.url;
      return `${url} ${v.width}w`;
    }).join(", ");
    const type = variants[0].format;
    return `<source type="image/${type}" srcset="${srcset}" sizes="${options?.sizes ?? "90vw"}">`;
  }).join("\n");
  ...
  const imgTag = `
        ...
        />
    `.trim();
  const wrapperStyle = `width: 100%; aspect-ratio: ${aspectRatio}; background-color: var(--bg-secondary);`;
...
function initGallery() {
  ...
  const images = Array.from(galleryItems).map((img) => {
    ...
    return {
      ...
      id: button?.id,
      metadata: button?.dataset.imageMetadata ? JSON.parse(button.dataset.imageMetadata) : null
    };
  });
  ...
  function updateLightboxImage(index) {
    ...
    if (photo.metadata) {
      ...
      lightboxContent.innerHTML = `<img id="lightbox-img" src="${photo.src}" alt="">`;
      const img = lightboxContent.querySelector("img");
...
Plyr2.defaults = cloneDeep(defaults_default);
export {
  generateImgTag,
  initGallery
};
